...
class wizard_giornale(models.TransientModel):
    ...
    def print_giornale(self, cr, uid, ids, context=None):
        ...
        move_ids = self.pool['account.move'].search(cr, uid,
            [
                ('date', '>=', wizard.date_move_line_from),
                ('date', '<=', wizard.date_move_line_to),
                ('state', 'in', target_type)
            ], order='date, id asc')
        if not move_ids:
        ...
        datas_form['fiscalyear_id'] = wizard.fiscalyear.id
        datas_form['print_state'] = 'draft'
        report_name = 'l10n_it_central_journal.report_account_central_journal'
        datas = {
            'ids': move_ids,
            'model': 'account.move',
            'form': datas_form
