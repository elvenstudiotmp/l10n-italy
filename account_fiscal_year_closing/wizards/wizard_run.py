...
class wizard_run(osv.osv_memory):
    ...
    def run(self, cr, uid, ids, context=None):
        ...
        pool = self.pool
        fyc_model = 'account_fiscal_year_closing.fyc'
        fyc_pool = pool.get(fyc_model)
        ...
        # Read the object
        fyc = fyc_pool.browse(cr, uid, active_id, context=context)
        ...
        #
        if fyc.create_opening or \
           fyc.create_closing or \
           fyc.create_net_loss_and_profit or \
           fyc.create_loss_and_profit:
            workflow_op = 'run'
        else:
            workflow_op = 'cancel'

        wf_service = netsvc.LocalService("workflow")
        wf_service.trg_validate(uid, fyc_model, fyc.id, workflow_op, cr)
    ...
    def _check_invalid_period_moves(self, cr, uid, fyc, context):
        ...
        """
        move_cls = self.pool.get('account.move')
        ...
        if len(account_move_ids):
            invalid_period_moves = move_cls.browse(
    ...
    def _check_unbalanced_moves(self, cr, uid, fyc, context):
        ...
        """
        move_cls = self.pool.get('account.move')
        dp_cls = self.pool.get('decimal.precision')
        ...
        #
        unbalanced_moves = []
        for move in move_cls.browse(cr, uid, account_move_ids, context):
            amount = 0
            for line in move.line_id:
                amount += (line.debit - line.credit)

            if round(abs(amount), dp_cls.precision_get(cr, uid, 'Account')) > 0:
    ...
    def create_closing_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'loss_and_profit':
            ...
            allowed_ops = ('loss_and_profit', 'net_loss_and_profit', 'close')
            assert_msg = _("The operation must be a supported one")
            assert operation in allowed_ops, assert_msg

        #
        # For each (parent) account in the mapping list
        #
        account_obj = pool.get('account.account')
        dp_obj = pool.get('decimal.precision')
        ...
        for account_map in account_mapping_ids:
            ...
            for account in account_obj.browse(cr, uid, child_ids, ctx):
                ...
                if account.type != 'view':
                    ...
                    if round(abs(balance), precision) > 0:
                        #
        ...
        _log.warning(operation)
        _log.warning(move_lines)
        #
        ...
        values = None
        if operation == 'loss_and_profit':
            values = {'loss_and_profit_move_id': move_id}

        elif operation == 'net_loss_and_profit':
            values = {'net_loss_and_profit_move_id': move_id}

        elif operation == 'close':
            values = {'closing_move_id': move_id}
        ...
        if values:
            fyc_obj = pool.get('account_fiscal_year_closing.fyc')
            fyc_obj.write(cr, uid, [fyc.id], values, context)
    ...
    def remove_move(self, cr, uid, operation, fyc, context):
        ...
        """
        pool = self.pool
        fyc_pool = pool.get('account_fiscal_year_closing.fyc')
        ...
        if operation == 'loss_and_profit':
            ...
            values = {'loss_and_profit_move_id': None}
        elif operation == 'net_loss_and_profit':
            move = fyc.net_loss_and_profit_move_id
            values = {'net_loss_and_profit_move_id': None}
        elif operation == 'close':
            move = fyc.closing_move_id
            values = {'closing_move_id': None}
        elif operation == 'open':
            move = fyc.opening_move_id
            values = {'opening_move_id': None}
        else:
        ...
        fyc_pool.write(cr, uid, fyc.id, values)
